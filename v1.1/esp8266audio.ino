...
double last_light_final[SAMPLE_SIZE]; //存储近200次采样的最终亮度
double hue = 0; //灯的颜色(HSV)
double hue_max = 2;
double hue_min = -112;
double delta_hue = -0.1;
int light_lock = 0; //防闪烁锁，当灯的亮度达到0以后，会被锁住，直到给定的输入亮度大于light_lock_threshold才会解锁
int light_lock_threshold = 12;
float lightScale = 5.5; //亮度和音量的放大倍数
int H,S,V;
...
//各弦钮对应的ws2812 led的实际编号,0~5代表6弦到1弦
int led_mapping[6] = {2,4,6,3,7,5};
...
void loop() {
  ...
  light_avg_short = light_avg_short/10;
  ...
  light_avg_long = light_avg_long/30;

  double light_avg_final = max(light_avg_long,light_avg_short); 
  last_light_final[count] = light_avg_final;//这个计算出来的亮度会被存入200次数组
  ...
  Serial.println(light_avg_final);
//  for(int i=0;i<SAMPLE_SIZE;i++){
//    Serial.print(last_light_final[i]);
//    Serial.print(" ");
//  }
//  Serial.println(" ");
  if(light_lock!=0){
    ...
    for(int i=0;i<6;i++){
      H=(int(floor(hue+i*6+255)))%255;
      double light_i = last_light_final[((count-i*10+SAMPLE_SIZE)%SAMPLE_SIZE)];
      double light_nor = light_i/255.0;
      S=floor((0.64-(1.6*light_nor-0.8)*(1.6*light_nor-0.8))*255.0);
      V=floor(light_i);
      leds[led_mapping[i]] = CHSV(H, S, V); //用HSV色彩空间，不断改变H即可
    }
  }
  ...
  hue += delta_hue;
  if(hue>hue_max || hue<hue_min) delta_hue = -delta_hue;
